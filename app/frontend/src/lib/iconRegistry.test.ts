...
describe("iconRegistry", () => {
  it("has 59 entries with a matching by-id map", () => {
    expect(ICON_REGISTRY).toHaveLength(59);
    expect(ICON_BY_ID.size).toBe(59);
  ...
  it("has the expected per-category counts (primary 22 / bespoke 31 / art 3 / brand 3)", () => {
    const counts = ICON_REGISTRY.reduce<Record<string, number>>((acc, entry) => {
      acc[entry.category] = (acc[entry.category] ?? 0) + 1;
      return acc;
    }, {});
    expect(counts).toEqual({ primary: 22, bespoke: 31, art: 3, brand: 3 });
  });
