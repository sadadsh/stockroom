...
describe("iconRegistry", () => {
  it("has 72 entries with a matching by-id map", () => {
    ...
    expect(ICON_REGISTRY).toHaveLength(72);
    expect(ICON_BY_ID.size).toBe(72);
  ...
  it("has the expected per-category counts (primary 32 / bespoke 34 / art 3 / brand 3)", () => {
    ...
    expect(counts).toEqual({ primary: 32, bespoke: 34, art: 3, brand: 3 });
  });
