...
describe("Components Design Studio scenarios", () => {
  ...
  it("shows every provider as one compact choice in Manage Models", async () => {
    ...
    const providerList = within(screen.getByRole("tablist", { name: "CAD model providers" }));
    expect(providerList.getAllByRole("tab")).toHaveLength(5);
    expect(providerList.getByRole("tab", { name: "Ultra Librarian" })).toBeEnabled();
    expect(providerList.getByRole("tab", { name: "SnapMagic" })).toBeEnabled();
    expect(providerList.getByRole("tab", { name: "SamacSys" })).toBeEnabled();
    expect(providerList.getByRole("tab", { name: "TraceParts" })).toBeEnabled();
    expect(providerList.getByRole("tab", { name: "CADENAS" })).toBeDisabled();
    expect(liveRequest).not.toHaveBeenCalled();
  });
});
