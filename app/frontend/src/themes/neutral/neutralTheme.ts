...
export const neutralTheme = defineTheme({
  ...
  // Stockroom is a native Windows engineering tool. Keep the copied neutral theme's hierarchy,
  // but use the same platform face and dense scale as the established shell rather than adding a
  // branded webfont that rasterises differently from surrounding desktop chrome.
  typography: {
    scale: {base: 12, ratio: 1.16},
    body: {
  ...
  // Controls change tone without springing or drifting away from the Win32 interaction idiom.
  motion: {fast: 100, medium: 180, slow: 400, ratio: 0.75},
  ...
  tokens: {
    ...
    // Accent + neutral surface tints follow Stockroom's authored grayscale ladder.
