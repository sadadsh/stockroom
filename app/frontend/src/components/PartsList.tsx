...
import { categoryHue } from "../lib/categoryHue";
...
export function RowThumbnail({ id, category }: { id: string; category: string }) {
  ...
  // The rendered-model image keeps the plain neutral tile; the FALLBACK glyph gets the category
  // tint (a faint hued fill + hairline) so kinds separate at a glance without a rendered thumbnail.
  const tile = thumb ? "border-line bg-field" : hue.tileClass || "border-line bg-field";
  return (
    <span
      ref={ref}
      style={thumb ? undefined : { color: hue.stroke }}
      className={`flex h-[30px] w-[30px] flex-none items-center justify-center overflow-hidden rounded-[7px] border ${tile}`}
    >
      {thumb ? (
...
// A small category glyph for the row thumbnail (north-star .rthumb): the part seen as its kind at
// a glance. The stroke uses currentColor, which RowThumbnail sets to the part's category hue
// (categoryHue) so families read apart (FIX-03); an unknown category inherits the neutral --c-t2.
// Falls back to a generic chip for a category with no dedicated glyph.
function CategoryGlyph({ category }: { category: string }) {
  const c = category.toLowerCase();
  const p = { fill: "none", stroke: "currentColor", strokeWidth: 1.6, strokeLinecap: "round" as const, strokeLinejoin: "round" as const };
  if (c.includes("resistor"))
    return (
      <svg viewBox="0 0 32 18" className="h-3.5 w-6" {...p}>
