...
upgrade is reverified automatically.
"""
...
def _write_receipt(
    ...
    *,
    ...
    payload = {"schema": _RECEIPT_SCHEMA, "libraries": [entry]}
...
def converge_altium_library(
    ...
    *,
    ...
    if dblib is None:
        return AltiumConvergenceResult(
            "no-library",
            "The active profile has no Altium database library yet.",
            receipt_path=str(receipt),
        )
    target = Path(dblib)
    if not target.exists():
        return AltiumConvergenceResult(
            "no-library",
            f"The active profile has no generated database library at {target.as_posix()}.",
            dblib=str(target),
            receipt_path=str(receipt),
        )
