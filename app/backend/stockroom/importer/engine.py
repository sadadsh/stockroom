...
from stockroom.model.sourced import SOURCED_DIRNAME, sourced_file, write_payload
...
def import_part(
    ...
    *,
    ...
    try:
        todo = wanted if refetch else needs_sources(library_root, record.id, wanted)
    except ValueError as exc:
        # An id the sourced layer refuses as a path component. MEASURED 2026-07-27 against the
        # owner's real library: all 158 records still carry the PRE-V3 id scheme
        # (`103at_2`, underscores), while `sourced/` only accepts the decided `[a-z0-9-]` form -
        # so evidence cannot be filed for any of them until the ids are migrated.
        #
        ...
        result.detail = (
            f"{exc}. This record predates the v3 id scheme (slug(mpn)+'-'+sha256(mpn)[:4]); "
            f"`sourced/` will not accept it as a path. The library needs an id migration first."
        )
        return result
    if not todo:
    ...
    if not fetched:
        result.outcome = Outcome.DEFERRED if result.deferred else Outcome.NO_DATA
        result.detail = (
            f"{', '.join(result.deferred)} refused for quota reasons; retryable"
            if result.deferred
            else "no usable source knows this MPN"
        )
        return result
    ...
    if dry_run:
        ...
        result.detail = "would write " + ", ".join(
            f"{SOURCED_DIRNAME}/{record.id}/{name}.json" for name in sorted(fetched)
        )
        return result
    ...
    result.outcome = Outcome.IMPORTED
    result.detail = "evidence stored and re-derived from " + ", ".join(sorted(fetched))
    return result
...
def run_import(
    ...
    *,
    ...
    usable = {name for name, _ in sources}
