...
def _rec(mpn="TPS62130RGTR", purchase=(), specs=None) -> PartRecord:
    return PartRecord(
        id="p1",
        display_name=mpn,
        category="ICs",
...
class FakePipeline:
    ...
    def attach_assets(self, part_id, candidate):
...
def test_falls_back_to_the_online_catalogue_when_the_record_has_no_id():
    pipe = FakePipeline(candidates=[_cand()])
    src = LcscSource(lambda: pipe, resolve_online=lambda mpn: "C41040738")
    src.supply(_rec())
...
def test_attaches_the_converted_files_under_the_real_manufacturer_part_number():
    ...
    pipe = FakePipeline(candidates=[_cand()])
    src = LcscSource(lambda: pipe, resolve_online=lambda mpn: "C7666")
...
def test_a_fresh_pipeline_is_built_per_part():
    ...
    src = LcscSource(make, resolve_online=lambda mpn: "C7666")
    src.supply(_rec())
    src.supply(_rec())
    assert len(made) == 2
...
def test_the_attach_is_pushed_through_the_supplied_write_lane():
    ...
    src = LcscSource(lambda: pipe, resolve_online=lambda mpn: "C7666", run_write=write_lane)
    src.supply(_rec())
    assert ran == ["write"]
    assert pipe.attached
...
def test_the_engine_marks_a_part_completed_once_the_source_has_attached():
...
def test_a_source_claiming_success_it_did_not_deliver_cannot_fake_a_completion():
    ...
    rec = _rec()
    pipe = FakePipeline(candidates=[_cand()])
    pipe.attach_assets = lambda part_id, candidate: None  # attaches nothing at all
    src = LcscSource(lambda: pipe, resolve_online=lambda mpn: "C7666")
    item = complete_part("p1", load_record=lambda _pid: rec, sources=[src])
