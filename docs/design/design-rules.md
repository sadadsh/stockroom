...
Each rule is a specific thing that made the app read as automation-made. The pattern is always
...
4. **No letterspaced UPPERCASE micro-labels sprinkled around.** `SIDE · ADG714 TERMINAL
   · SWITCHED ROLE · PIN NAMES` everywhere is an automation tell and adds noise.
...
5. **No colored accent bar / rail on a rounded card.** This specific combination is on
   every "looks automation-made" list. Encode class/category some other way (a small dot, the
