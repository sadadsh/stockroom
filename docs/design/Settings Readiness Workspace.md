...
| Library | Independent library repositories, collaborator sync, per-user GitHub access | App updates or another library's history |
...
- Repository activation invalidates library, system, part, facet, and Altium
  state. One library repository can never be committed into another or into the
  Stockroom application repository.
- Library sync refreshes its status and, after a pull, invalidates library and
  Altium projections.
- GitHub identity is owned by Git Credential Manager for the current Windows
  user; Stockroom stores no personal access token. GitHub repository permissions
  are the collaboration authority, so app access never grants library access.
